...
export function usePricingCalculator() {
  ...
  const calculatePricing = async (description) => {
    ...
    try {
      ...
      if (keywords.includes('ai') || keywords.includes('machine learning')) baseRate += 20
      if (keywords.includes('blockchain') || keywords.includes('crypto')) baseRate += 15
      if (keywords.includes('mobile') || keywords.includes('app')) baseRate += 10
